...
static void ril_powerd_set_display_state(struct ofono_powerd *powerd,
					...
					void *user_data)
{
	...
	ret = g_ril_send(pwrd->ril, RIL_REQUEST_SCREEN_STATE, &rilp,
				ril_display_state_cb, cbd, g_free);
	if (ret <= 0) {
		g_free(cbd);
...
static int ril_powerd_probe(struct ofono_powerd *powerd,
					unsigned int vendor,
					void *user)
{
	GRil *ril = user;
...
static void ril_powerd_remove(struct ofono_powerd *powerd)
{
	struct powerd_data *pwrd = ofono_powerd_get_data(powerd);
